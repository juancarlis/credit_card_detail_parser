...
def extract_data(text: List[str]) -> pd.DataFrame:
    ...
    for line in text:
        ...
        if date_match:
            ...
            if value_matches:
                ...
                value = (
                    value_matches[-2]
                    if value_matches[-1] == "0,00"
                    else value_matches[-1]
                )
    ...
    # Create a DataFrame
    data = pd.DataFrame(
        {"Fecha": dates, "Descripcion": descriptions, "Valores": values}
    )
...
def save_to_csv(df: pd.DataFrame, file_path: Path) -> None:
    ...
    try:
        df.to_csv(file_path, encoding="utf-8-sig", sep=";", decimal=",", index=False)
